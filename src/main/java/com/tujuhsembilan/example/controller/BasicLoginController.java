...
public class BasicLoginController {
 ...
 @PostMapping("/login")
  public ResponseEntity<?> login(@NotNull Authentication auth) {
    ...
    long accessTokenExpiry = 3600L; // 1 hour validity

    long refreshTokenExpiry = 7 * 24 * 3600L; // 7 days validity for Refresh Token
    ...
    var refreshToken = jwtEncoder.encode(JwtEncoderParameters.from(
        JwsHeader.with(SignatureAlgorithm.ES512).build(),
        refreshClaims));
  ...
  @DeleteMapping("/logout")
   @PreAuthorize("isAuthenticated()")
    public ResponseEntity<?> logout(@RequestBody String token) {
        ...
        invalidTokens.put(token, Instant.now());
        return ResponseEntity.ok("Logged out successfully");
    }

    public boolean isTokenValid(String token) {
        // Check if the token is in the invalid tokens store
        return !invalidTokens.containsKey(token) || invalidTokens.get(token).isBefore(Instant.now());
    }
}